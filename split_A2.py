...
df = pd.read_excel("./data/article/A2_二次標註_網路危機訊息標註_1100927_僅全文標註(給致寧).xlsx")
df['split_sentences'] = df.apply(lambda row: [sent for sent in split_sentence(row['Content(remove_tag)']) if sent not in ['', ' ', '。', '，', '\n']], axis=1)
...
output_df.to_excel('./data/article/split_output_A2_1113.xlsx')
