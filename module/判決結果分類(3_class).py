...
df_train['Result_ori'] = df_train['Result']
df_val['Result_ori'] = df_val['Result']
df_test['Result_ori'] = df_test['Result']
...
swap_result_map = {np.array([1,0,0], dtype=df_train['Result_ori'][0].dtype).tobytes():np.array([0,1,0]),                    np.array([0,1,0], dtype=df_train['Result_ori'][0].dtype).tobytes():np.array([1,0,0]),                    np.array([0,0,1], dtype=df_train['Result_ori'][0].dtype).tobytes():np.array([0,0,1])}

print(swap_result_map.keys())
...
def dropReasoning(x, selected_list=None):
    if selected_list is not None:
        if x.name in selected_list:
            # print(x)
            for i in ['AA_concat', 'AD_concat', 'RA_concat', 'RD_concat']:
                x[i] = np.nan
            # x = x.drop(labels=['AA_concat', 'AD_concat', 'RA_concat', 'RD_concat'])
            # print(x)
            
            # change result
            # print("origin", x['Result'])
            x['Result'] = np.array([0,0,1])
            # print("swap", x['Result'])
...
def identicalReasoning(x, selected_list=None):
    if selected_list is not None:
        if x.name in selected_list:
            # print(x.index[x.index.str.contains('(AA|RA)(?!_neu|_concat)')])
            
            # merge AA and RA
            tmp_list = []
            advantage_rule_list = ['(AA|RA)', '(AA)', '(RA)']
            #advantage_rule_list = ['(AA)', '(RA)']
            advantage_rule = shuffle(advantage_rule_list, random_state=6789)[0]
            x['advantage_rule'] = advantage_rule
            for i in x.index[x.index.str.contains(advantage_rule+'(?!_neu|_concat)')]:
                # print(x[i])
                if not np.isnan(x[i]).any():
                    tmp_list.append(x[i])
                    
            # print(len(tmp_list))
            x['AA_concat'] = np.nanmean(tmp_list, axis=0)
            x['RA_concat'] = x['AA_concat']
            # print(len(x['RA_concat']))
            
            # merge AD and RD
            tmp_list = []
            disadvantage_rule_list = ['(AD|RD)', '(AD)', '(RD)']
            #disadvantage_rule_list = ['(AD)', '(RD)']
            disadvantage_rule = shuffle(disadvantage_rule_list, random_state=7890)[0]
            x['disadvantage_rule'] = disadvantage_rule
            for i in x.index[x.index.str.contains(disadvantage_rule+'(?!_neu|_concat)')]:
                # print(x[i])
                if not np.isnan(x[i]).any():
                    tmp_list.append(x[i])
                    
            # print(len(tmp_list))
            x['AD_concat'] = np.nanmean(tmp_list, axis=0)
            x['RD_concat'] = x['AD_concat']
            # print(x['RD_concat'])
...
for name in df_name_list:
    ...
    # select identical reasoning list
    ## identical and swap use same partial_list
    partial_list = shuffle(total_list, random_state=7788)
    selected_length = int(len(partial_list)*identical_reasoning_rate)
    identical_reasoning_list = partial_list[0:selected_length]
    tmp_dict['identical_reasoning'] = identical_reasoning_list
    
    # select drop reasoning cases
    partial_list = shuffle(total_list, random_state=7890)
    selected_length = int(len(partial_list)*drop_reasoning_rate)
    drop_reasoning_list = partial_list[0:selected_length]
    tmp_dict['drop_reasoning'] = drop_reasoning_list
...
for name in df_name_list:
    ...
    if name == 'train':
        ...
        # random identical reasoning
        print("identical reasoning...")
        df_dict[name] = df_dict[name].apply(identicalReasoning, selected_list=random_selected_row_index_dict[name]['identical_reasoning'], axis=1)
        
        # random drop reasoning
        ## drop should be the last opertation, because drop will remove 'AA_concat', 'AD_concat', 'RA_concat', 'RD_concat',
        ## if drop before any other operation, those operation become useless
        print("drop reasoning...")
        df_dict[name] = df_dict[name].apply(dropReasoning, selected_list=random_selected_row_index_dict[name]['drop_reasoning'], axis=1)
...
print(df_dict['train'].iloc[3].to_string())
...
for i in range(len(X_dict_of_list['train'])):
    ...
    y_dict_of_list['train_res'].append(y_train_res)
...
model = splited_DNN(feed_dict['x'], feed_dict['y'], num_or_size_splits=docvec_size, bottleneck_size=60)

model.print_cost(feed_dict=feed_dict)
# model.predict(feed_dict['x'])
# model.predict_multi_label(feed_dict['x'])
# ### Training
...
#     epoch = random.randrange(5, 80)
    ...
    #     print('multiple:', i+1)
        ...
        accuracy, roc_auc, pr_auc, f1 = print_score(model, X_dict_of_list['test'][i], y_dict_of_list['test'][i])
        print('\n')
...
#         roc_auc_str = ''.join('{}-{}_'.format(key, val) for key, val in roc_auc_str.items())
        ...
        modelFilePath = ("./models/SplitDNN判決結果分類/"
            ...
            "_roc_auc{}_pr_auc{}_f1_{}__seed{}_{}.ckpt").format(
                class_num,
...
model = splited_DNN(np.zeros(shape=(1,800)), np.zeros(shape=(1,3)), num_or_size_splits=[200, 200, 200, 200], bottleneck_size=60)
