...
# label_path = './data/final/article/A1A2_article.xlsx'
...
# sentence_path = './data/final/article/split_A1A2_article_answer.xlsx'
...
# # count statistic of each sentence label
# stat = [0]*4
# for i in range(4):
#     stat[i] = s_df[s_df[sentence_label_list[i]] == 1].shape[0]
# print("statistic of each sentence label: ", stat)
...
#     # print(article_dict[str(row['TextID'])]['label'].values[0] == "b:B")
...
# # all_article_df.to_excel('data/final/article/A1A2_article_with_sentences_split.xlsx', index=False)
# print("save done!")
...
# all_df = pd.read_excel('data/final/article/A1A2_article_with_sentences_split.xlsx')
...
#     all_train_df[i].to_excel(f'data/final/article/A1A2_train_fold_{i+1}.xlsx', index=False)
#     all_test_df[i].to_excel(f'data/final/article/A1A2_test_fold_{i+1}.xlsx', index=False)


# In[8]:
for i in range(1,6):
    without_test_df = pd.read_excel(f'data/final/article/A1A2_train_fold_{i}.xlsx')
    df_crisis_level_0 = without_test_df.loc[without_test_df['Crisis_Level'] == 0]
    df_augmentation_0 = df_crisis_level_0.sample(n=200, random_state=seed)
    df_remaining = without_test_df.drop(df_augmentation_0.index) 

    # 當作Augmentation的資料，將Crisis_Level為0(無危機)的資料取出
    df_augmentation_0.to_excel(f'data/final/article/A1A2_be_augment_0_0530_fold_{i}.xlsx', index=False)

    df_crisis_level_1 = df_remaining.loc[without_test_df['Crisis_Level'] == 1]
    df_augmentation_1 = df_crisis_level_1.sample(n=50, random_state=1)
    df_remaining_v2 = df_remaining.drop(df_augmentation_1.index) 
    df_augmentation_1.to_excel(f'data/final/article/A1A2_be_augment_1_0530_fold_{i}.xlsx', index=False)


    df_remaining_v2.to_excel(f'data/final/article/A1A2_train_moveAug_0530_fold_{i}.xlsx', index=False)
