...
# factor_list 可以不要
parser.add_argument("--factors", type=list, default=[])
parser.add_argument("--pred_data", type=str, default="")
# 要不要再斷詞一遍
parser.add_argument("--do_segment", action='store_true') 
# 有沒有中性句
parser.add_argument("--neutral_data", type=str, default="sex")
# Model 的版本
...
'train_data': args.train_data, 'pred_data':args.pred_data, 'neutral_data': args.neutral_data, 'factor_lst': args.factors}
print('>>>>> All model setting: ', model_setting)

if args.do_segment:
    bar = progressbar.ProgressBar()
    for i in bar([model_setting['train_data'], '_'.join([model_setting['train_data'], model_setting['train_data']])]): # df, df_neu
